...
The Jekyll repo (`siddhantsomani.github.io`) had one real post ("HTTP Servers and Related Concepts") which was ported into `src/content/posts/en/http-servers-and-related-concepts.md`. That repo's content has been replaced with a single static `index.html` that meta-refreshes + JS-redirects to `https://siddhant.blog/` (so existing `siddhantsomani.github.io` bookmarks keep working), and the repo is archived. `.nojekyll` is committed so Pages serves the redirect verbatim instead of running a Jekyll build.
